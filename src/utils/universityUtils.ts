...
export function getMergedUniversities(): Record<string, UniversityInfo[]> {
  ...
  partnersData.forEach((p: any) => {
    let rawCountry = p["國別"] || "";
    // Clean up country name, e.g. "Japan 日本" -> "日本", "Canada 加拿大" -> "加拿大"
    // Usually the Chinese name is at the end. We can just use the raw string, or extract Chinese.
    let country = rawCountry;
    if (rawCountry.includes("日本")) country = "日本";
    else if (rawCountry.includes("美國") || rawCountry.includes("USA")) country = "美國";
    ...
    else country = rawCountry; // default

    if (!merged[country]) {
      merged[country] = [];
    }

    const uniName = `${p["合作學校(中文名)"]} (${p["合作學校(英文名)"]})`;
    
    // Check if already exists
    const exists = merged[country].find(u => u.name === uniName || u.name.includes(p["合作學校(英文名)"]));
    if (!exists) {
      merged[country].push({
        name: uniName,
