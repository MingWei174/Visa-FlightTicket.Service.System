...
const mangledStartStr = "globalCountry={globalCountry}";
const mangledEndStr = "比價機票\n                            </button>\n                          </div>\n                        </div>\n                      </div>\n\n                    </div>";

const startIdx = app.indexOf(mangledStartStr);
const endIdx = app.indexOf(mangledEndStr) + mangledEndStr.length;

if (startIdx === -1 || app.indexOf(mangledEndStr) === -1) {
  console.log("Could not find mangled block!");
  process.exit(1);
}

const mangledBlock = app.substring(startIdx, endIdx);
...
console.log('Restored fully successfully.');
